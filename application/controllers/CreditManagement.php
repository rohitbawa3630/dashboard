...
class CreditManagement extends CI_Controller
{
	...
	public function parseData($document_id)
	{
				...
				curl_setopt_array($curl, array(
				  CURLOPT_URL => "https://api.docparser.com/v1/results/nzoairljowvn/$document_id",
	...
	public function UploadPdfByUrl($filename)
	{
			...
			curl_setopt_array($curl, array(
			  CURLOPT_URL => "https://api.docparser.com/v1/document/fetch/nzoairljowvn",
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
			  ...
			  CURLOPT_POSTFIELDS => "url=https://stagingapp.pickmyorder.co.uk/credits/$filename",
			  CURLOPT_HTTPHEADER => array(
				"Authorization: Basic MGE0ZjUyNmY0ZWNmNDRmOTdjZTMwMmQxNjVhZWM3MDkzYWNlNjVmOTo=",
	...
	public function FetchCreditAndStore()
	{
		$TableData=array();
        ...
        $emailData = imap_search($connection, 'SUBJECT "Credit"');
					  ...
					  $attachments[$i] = array(
											...
											if($attachments[$i]['is_attachment']) {
												...
												//	$attachments[$i]['attachment'] = base64_decode($attachments[$i]['attachment']);
	                                                $AllInvoicesid=$this->db->query("select id from credits where dateAndTime='$emaildateTime'");
	...
	{
		$filename=rand()*time().$get."credit.pdf";
		$url=base_url().$filename;
		if(file_put_contents("./credits/$filename",$base[$get]['data']))
		{
			...
			$AllInvoicesNumber=$this->db->query("select id from credits where invoice_number='$invoice_number'");
