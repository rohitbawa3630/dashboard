...
class QuotesManagent extends CI_Controller
{
	...
	public function parseData($document_id)
	{
				...
				curl_setopt_array($curl, array(
				  CURLOPT_URL => "https://api.docparser.com/v1/results/ghxpsxzwnrlb/$document_id",
	...
	public function UploadPdfByUrl($filename)
	{
			...
			curl_setopt_array($curl, array(
			  CURLOPT_URL => "https://api.docparser.com/v1/document/fetch/ghxpsxzwnrlb",
			  CURLOPT_RETURNTRANSFER => true,
			  CURLOPT_ENCODING => "",
			  CURLOPT_MAXREDIRS => 10,
			  CURLOPT_TIMEOUT => 0,
			  ...
			  CURLOPT_POSTFIELDS => "url=https://stagingapp.pickmyorder.co.uk/quotes/$filename",
			  CURLOPT_HTTPHEADER => array(
				"Authorization: Basic MGE0ZjUyNmY0ZWNmNDRmOTdjZTMwMmQxNjVhZWM3MDkzYWNlNjVmOTo=",
	...
	public function FetchQuotesAndStore()
	{    
		$TableData=array();
        ...
        $emailData = imap_search($connection, 'SUBJECT "Quotation"');
					  ...
					  $attachments[$i] = array(
											...
											if($attachments[$i]['is_attachment']) {
												...
												//	$attachments[$i]['attachment'] = base64_decode($attachments[$i]['attachment']);
	                                                $AllInvoicesid=$this->db->query("select id from quotesInvoice where dateAndTime='$emaildateTime'");
...
$reverse=count($base)-1;
	for($get=0;$get<count($base);$get++)
	{
		$filename=rand()*time().$get."quotes.pdf";
		$url=base_url().$filename;
		if(file_put_contents("./quotes/$filename",$base[$reverse]['data']))
		{
			...
			$invoice_number=json_decode(json_encode($decodedData[$get]->invoice_number));
			
			$AllInvoicesNumber=$this->db->query("select id from quotesInvoice where invoice_number='$invoice_number'");
			...
			$totals=json_decode(json_encode($decodedData[$get]->totals),true);
					...
					$this->db->insert('quotesInvoice',$data);
			}
		}	
$reverse=$reverse-1;		
	}
