...
$Orderdata=$_POST['cart'];
//card details
$CardNumber=$_POST['card'];
$ExpDate=$_POST['exp_month'];
$ExpYear=$_POST['exp_year'];
$CardType=$_POST['brand'];
$SaveStatus=$_POST['savestatus'];
$WholsalerBusinessID=$_POST['WholsalerBusinessId'];
$key='sk_live_Ke4mJg5JFJ45r406DpKtYIpb'; //set bydefault key
$isconnected=0;
/*******************************************Get Secret key code *******************************************/
			if(!$WholsalerBusinessID) //if a simple app user select wholsaler product then need wholsaler id to send money 
			{
			$GetUserData=json_decode($Orderdata);
			$user_array=$GetUserData->Orderdetails;
			$engineer_id=$user_array[0]->user_id;;
			...
			{
				$Busines_id=$WholsalerBusinessID;
			}
			...
			if($get_bussiness_details1[0]['isdedicate']) //check if the account is dedicate
			{
				$key=$get_bussiness_details1[0]['stripe_secretkey'];
				if(!$key)
				{
				$key='sk_live_Ke4mJg5JFJ45r406DpKtYIpb';
				}
			}
			if($get_bussiness_details1[0]['isconnected']) //check if the account is connected
			{
				$isconnected=$get_bussiness_details1[0]['isconnected'];
				$wholsellerAccountId=$get_bussiness_details1[0]['stripe_user_id'];
...
$j=0;
$allid=array();
if(isset($_POST['amount']) && isset($_POST['token']) && isset($_POST['currency']) && isset($_POST['description']) && isset($_POST['cart']) && isset($_POST['card']) && isset($_POST['exp_month']) && isset($_POST['exp_year']) && isset($_POST['brand'])&& isset($_POST['savestatus']))
{
		...
		CURLOPT_POSTFIELDS => "amount=$amount&currency=$cur&source=$token&description=$description",
		...
		$ConvertedJson= json_decode($response,true);
		
		if($ConvertedJson['error'])
		{
			
			echo json_encode(array("statusCode"=>401,"valid"=>false,'Message'=>$ConvertedJson['error']['message']),JSON_FORCE_OBJECT);
		}
		else
		{ 
	          $charge_id= $ConvertedJson['id'];
			/************************transfer payment *******************/
			if($isconnected)
			{
				$curl = curl_init();
					curl_setopt_array($curl, array(
					  ...
					  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
					  CURLOPT_CUSTOMREQUEST => "POST",
					  CURLOPT_POSTFIELDS => "amount=$amount&currency=$cur&source_transaction=$charge_id&destination=$wholsellerAccountId",
					  CURLOPT_HTTPHEADER => array(
						"Authorization: Bearer sk_live_Ke4mJg5JFJ45r406DpKtYIpb",
						"Content-Type: application/x-www-form-urlencoded",
						"Cookie: machine_identifier=dkPp9dXX06PAiT9LkDP7j1CgP8oH%2BuFeC0irGqLYv1UXjB6GWVJxLhKhfUYfeJ6%2FNw8%3D; private_machine_identifier=VdCwjh8gBIXpeBQ%2BC2a17khnMXaElAFY7PAk2yPnXX5LtJUX7B9zZNcLj43jtsVESPg%3D; __stripe_orig_props=%7B%22referrer%22%3A%22%22%2C%22landing%22%3A%22https%3A%2F%2Fconnect.stripe.com%2Foauth%2Ftoken%22%7D; cid=fd3e1ea3-62be-49e6-8acd-cca27a049e6c; stripe.csrf=3PyQzmYU08p2qJ1f2VQqcw03swF2Nd7dmSlIIovmgWyosP5DobuIIFwt16ZWlA55KSSKWcu4uRegjpjYFWmovQ%3D%3D"
					  ),
					));

					$response = curl_exec($curl); //need to put responce in db (pending)
...
/************************************************set collection and delivery*************************************************/
	...
	{	
		...
		padding-top: 10px;">'.$procode_new.'</td>
			 ...
			 $Busines_id=$Business_id[0]['business_id'];
			$user_email=$Business_id[0]['email'];
		  ...
		  $get_bussiness_details=$this->db->query("select * from dev_business where id='$Busines_id'");
		   ...
		   $insert=array('business_id'=>$Busines_id,'total_ex_vat'=>$total_ex,'date'=>$date,'po_reffrence'=>$po_reffrence,'total_inc_vat'=>$total_inc,'status'=>$status,'engineer_id'=>$engineer_id,'supplier_id'=>$supplier_id,'instruction'=>$delivery_instruction,'odrdescrp'=>$order_desc,'givenprojectid'=>$project_id,'givenprojectname'=>$given_project_name,'po_number'=>$startorder,'Transaction_id'=>$ConvertedJson['id'],'payment_status'=>$ConvertedJson['status']);
					...
					$atrdata=json_encode(array("details"=>$atradeyaarray));
					  ...
					  CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,
					  CURLOPT_CUSTOMREQUEST => "POST",
					  CURLOPT_POSTFIELDS => "sender=$engineer_id&order=$atrdata&paymentstaus=1&projectid=$project_id&collection=$collect&orderdes=$order_desc&instruction=$delivery_instruction&payment_id=$charge_id",
					  CURLOPT_HTTPHEADER => array(
	  ...
	  <td align="center">
		...
		<table width="600px" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
						<td  width="400px;"><h3 style="font-weight: bold;">PURCHASE ORDER-PAY BY CARD</h3>
						...
						<ul style="list-style:none; padding-left:0px;">
						  <li style="padding-bottom: 5px;"><b>Purchase Order Date</b></li>
						   ...
						   <li style="padding-bottom: 5px;">'.$order_desc.'</li>
						     <li style="padding-bottom: 5px;"><b>Transaction Id</b>
						   <li style="padding-bottom: 5px;">'.$ConvertedJson['id'].'</li>
	...
	/***************************************invoice to user***************************************************/
	$invoice='<html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /><style>td {vertical-align: -webkit-baseline-middle;}</style></head>
	  ...
	  <td align="center">
		...
		<table width="600px" border="0" cellspacing="0" cellpadding="0" align="center">
				<tr>
						<td  width="400px;"><h3 style="font-weight: bold;">INVOICE-PAY BY CARD</h3>
							<ul style="list-style:none; padding-left:0px;display:'.$iswholeseller.'">
	...
	$headers .= 'Content-type: text/html; charset=UTF-8' . "\r\n";
			   ...
			   mail($user_email,'You Have An Order', $invoice,$headers);
			...
			if($SaveStatus=='1' && !in_array($user_id,$allid))         
			{   
			//Save user Card
		
				$PaymentData=array('user_id'=>$user_id,'cardnumber'=>$CardNumber,'exp_date'=>$ExpDate,'exp_year'=>$ExpYear,'card_type'=>$CardType);
				$InsertStatus=$this->DataModel->insert('dev_PaymentMethod',$PaymentData); 
			
			
			} 
		echo json_encode(array("statusCode"=>200,"valid"=>true,'Message'=>$ConvertedJson['status'],'transactionId'=>$ConvertedJson['id'],'orderid'=>$startorder),JSON_FORCE_OBJECT);
	} //end of order is sucess
	else
	{
	echo json_encode(array("statusCode"=>500,"valid"=>true,'Message'=>'Payment Successfully But Order Failed Due To Server Error please contact to Admin'),JSON_FORCE_OBJECT);
	}		
			
			
	} //End if payment goes success
}
else
{
