	...
	{	
		...
		foreach($get_product as $single)
		{
			...
			else  // Pick inclusive price from product table
			{
				...
				$status='1';
				$engineer_id= $user_id;
				$insert=array('project_id'=>$project_id,'total_ex_vat'=>$total_ex_price,'order_desc'=>$order_desc,'product_id'=>$single['id'],'qty'=>$Qty,'Reffrence_id'=>$po_reffrence,'status'=>'44','ex_vat'=>$single_ex_vat,'	vat_class'=>$taxclass,'variation_option_name'=>$variation_option_name,'variation_id'=>$variation_id ,'date'=>$date,'image'=>$single['products_images'],'ProductTitle'=>$procode_title_new,'ProductCode'=>$procode_new);
		...
		}else
		{
					...
					$status='1';
					$engineer_id= $user_id;
					$insert=array('project_id'=>$project_id,'total_ex_vat'=>$total_ex_price,'order_desc'=>$order_desc,'qty'=>$Qty,'Reffrence_id'=>$po_reffrence,'status'=>'44','ex_vat'=>$single_ex_vat,'vat_class'=>$taxclass,'variation_option_name'=>$variation_option_name,'variation_id'=>$variation_id ,'date'=>$date,'image'=>'http://app.pickmyorder.co.uk/images/applogo/pickapp.png','ProductTitle'=>$procode_title_new,'ProductCode'=>$procode_new);
					$in=$this->DataModel->insert($Single_order_table,$insert);
		}
